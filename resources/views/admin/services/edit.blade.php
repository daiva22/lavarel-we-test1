...
<head>
    <meta charset="UTF-8">
    <title>Edit Service - AutoTech</title>
...
<div class="main">
    ...
    <form method="POST" action="{{ route('admin.services.update', $service->id) }}" enctype="multipart/form-data">
        ...
        <textarea id="description" name="description" rows="5">{{ old('description', $service->description) }}</textarea>
        ...
        @if($service->image)
            <div style="margin-top:10px; margin-bottom:20px;">
                ...
                <img src="{{ asset($service->image) }}" alt="{{ $service->name }}" style="width:120px; height:120px; object-fit:cover; border-radius:8px; border:1px solid #ddd;">
            </div>
        @endif

        <label for="price">Price</label>
        <input type="number" id="price" step="0.01" min="0" name="price" value="{{ old('price', $service->price) }}" required>

        <label for="duration_minutes">Duration (minutes)</label>
        <input type="number" id="duration_minutes" min="1" name="duration_minutes" value="{{ old('duration_minutes', $service->duration_minutes) }}" required>
        ...
        <select id="mechanics" name="mechanics[]" multiple style="min-height: 140px;">
            @foreach($mechanics as $mechanic)
                <option
                    value="{{ $mechanic->id }}"
                    {{ in_array($mechanic->id, old('mechanics', $service->mechanics->pluck('id')->toArray())) ? 'selected' : '' }}>
        ...
        <div style="margin-top:20px; display:flex; gap:10px;">
            <button type="submit" class="btn-approve">Update Service</button>
